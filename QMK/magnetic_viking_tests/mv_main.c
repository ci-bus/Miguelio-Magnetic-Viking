...
static uint16_t     hall_last_value[MATRIX_COLS * MATRIX_ROWS] = {0};
static uint32_t     hall_diffs[MATRIX_COLS * MATRIX_ROWS] = {0};
...
uint16_t hall_get_sensor_value(uint8_t col_read, uint8_t row_read) {
    uint8_t index = (row_read * MATRIX_COLS) + col_read;
    uint16_t sum_values = 0;
        ...
        for (uint8_t col = 0; col < MATRIX_COLS; col++) {
            ...
            for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
                if (col == col_read && row == row_read) {
                    ...
                    count_reads ++;
                    sum_values += raw_value;
                    if (hall_last_value[index] != 0 && hall_last_value[index] != raw_value) {
                        hall_diffs[index] += abs(hall_last_value[index] - raw_value);
                    }
                    hall_last_value[index] = raw_value;
                } else {
    ...
    uprintf("Col: %u, Row: %u, diffs: %lu ", col_read, row_read, hall_diffs[index]);

    return sum_values / count_reads;
}
    ...
    for (uint8_t col = 0; col < MATRIX_COLS; col++) {
        ...
        for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
            if (valid_sensor(col, row)) {
                ...
                uint8_t compare_result = abs(all_sensors_average - hall_value_temp);
